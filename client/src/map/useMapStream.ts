// WebSocket client composable for the planning Map view's live updates.
//
// Mirrors `dag/useDagStream.ts` (same transport pattern, shared `epic:<id>`
...
// reducer (`map/stream.ts`) into a caller-provided reactive `MapState`. Owns
// the socket lifecycle (unsubscribe + close on unmount, bounded reconnect with
// backoff). The reducer holds all the state logic; this file is just
// transport + wiring, so it stays small and is unit-tested in isolation via
// the reducer.

import { getCurrentScope, onScopeDispose, ref, type Ref } from "vue";

import { applyMapFrame, type MapFrame, type MapState } from "./stream";
...
/**
 * Subscribe a reactive `MapState` to an epic's live map stream.
 *
